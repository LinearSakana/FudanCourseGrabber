...
> ⚠ **注意**
>
> 出于安全考量，本仓库根目录下 /captchaRecords 不会存储完整的验证码记录，仅有一些以 imgIndex 命名，储存 posIndex 与
> moveEndX 键值对的示例文件。您需要在本地配置好相关文件，并勾选 **导入本地 Captcha 查找表（.json）**，才能使用验证码查表功能
...
5. **验证码查表**：导入本地配置好的 .json 文件
6. **开始抢课**：设置并发数（在使用验证码的情况下，不建议设置为大于 2 的数字），在教务处释放余量的前十几秒开始抢课
